  ...
  <div class="m-product-card__content m:text-center">
    <div class="m-product-card__info">
      ...
      <div class="m-product-card__price">
        <div class="m-price ${sale ? 'm-price--on-sale ' : ''}m:inline-flex m:items-center m:flex-wrap">
          ${sale
...
function homePage({ banners, featured, posts, sections: cmsSections, settings }) {
  ...
  const bundleHtml = (bundleDeals.products || []).length ? `<section class="m-section m-section-my m-section-py db-bundle-section"><div class="container db-home-container">
    ...
    <div class="db-bundle-track" data-slider-track>${bundleDeals.products.map((product) => `<div class="db-bundle-track__item">${productCard(product, product.hover_image)}</div>`).join('')}</div>
  </div></section>` : '';
  ...
  const seenHtml = (seen.products || []).length ? `<section class="m-section db-as-seen-section"><div class="container-full">${sectionHeading(seen.title || '', seen.subtitle || '')}<div class="db-as-seen-track" data-slider-track>${seen.products.map(asSeenCard).join('')}</div></div>
  ...
  const reviewHtml = (reviews.items || []).length ? `<section class="m-section m-section-my m-section-py"><div class="container db-home-container">${sectionHeading(reviews.title || '', reviews.subtitle || '')}<div class="db-reviews" data-slider-track>${reviews.items.map((item) => {
  ...
  // Get Inspired — UGC grid using real products from the DB (section_products).
  const igHandle = '@majestic_nailbox';
  const socialProducts = (social.products || []).slice(0, 4);
  const socialHtml = socialProducts.length ? `<section class="m-section--tight db-home-social"><div class="container db-home-container">${sectionHeading(social.title || 'Get Inspired by Every Look', social.subtitle || 'Follow our studio for new sets and customer looks.')}<div class="db-insta-grid" data-slider-track>${socialProducts.map((p) => `<div class="db-insta-tile"><span class="db-insta-tile__handle">${e(igHandle)}</span><a href="/products/${e(p.slug)}" class="db-insta-tile__media"><img src="${e(p.image || PLACEHOLDER)}" alt="${e(p.title || 'Customer nail look')}" loading="lazy"></a><a class="db-insta-tile__shop" href="/products/${e(p.slug)}">Shop the Look</a></div>`).join('')}</div></div></section>` : '';
